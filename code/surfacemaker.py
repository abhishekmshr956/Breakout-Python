...
class SurfaceMaker:
    def __init__(self):
        ...
        for index, info in enumerate(walk('../graphics/blocks')):
            if index == 0:
                self.assests = {color:{} for color in info[1]}
            else:
                for image_name in info[2]:
                    color_type = list(self.assests.keys())[index-1]
                    full_path = '../graphics/blocks' + f'/{color_type}/' + image_name
                    surf = pygame.image.load(full_path).convert_alpha()
                    self.assests[color_type][image_name.split('.')[0]] = surf
